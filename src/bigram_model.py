...
def bigrams_count_to_probabilities(
    ...
    smooth_bigram = bigram_counts + smooth_factor  # De esta manera se cumple (smooth_factor*count(i,j))/(smooth_factor*|V| + sum(count(i,j)))

    suma_columnas = smooth_bigram.sum(dim=1, keepdim=True) #Es una columna de tensores

    suma_columnas[suma_columnas == 0 ] = 1.0

    probabilidades = smooth_bigram/suma_columnas
...
def calculate_neg_mean_log_likelihood(
    ...
    for word in words:
        total_log_likelihood += calculate_log_likelihood(word, bigram_probabilities, char_to_index, start_token, end_token).item()
...
def sample_next_character(
    ...
    next_char_index = torch.multinomial(current_probs, num_samples=1).item()      #basándote en una lista de probabilidades, puedes seleccionar un índice de manera aleatoria, pero respetando esas probabilidades
...
#Likelihood: Se usa cuando ya hemos observado algunos datos y queremos estimar qué tan bien un conjunto de parámetros explica esos datos.
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word)-1):
        big0 = processed_word[i]
        big1 = processed_word[i+1]

        pos_big0 = char_to_index[big0]
        pos_big1 = char_to_index[big1]

        bigram_prob = bigram_probabilities[pos_big0, pos_big1]
